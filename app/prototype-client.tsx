...
function EnterpriseProjects(){
  return <section className="page-stack project-archive-page">
    ...
    <article className="project-record-card">
      <header><div><span className="project-status">进行中</span><small>项目档案 · PRJ-202608-0001</small><h2>2026年第二批专精特新中小企业资质申报</h2><p>关联政策：{REAL_POLICY_TITLE}</p></div><div className="project-countdown"><b>6</b><span>天后截止</span><small>2026年8月31日 24:00</small></div></header>
      <dl className="project-meta"><div><dt>申报企业</dt><dd>北京星云科技有限公司</dd></div><div><dt>当前阶段</dt><dd>材料准备</dd></div><div><dt>项目负责人</dt><dd>周运营</dd></div><div><dt>档案更新时间</dt><dd>今天 15:42</dd></div></dl>
      <div className="project-progress" aria-label="项目进度"><div className="done"><i>✓</i><span><b>项目建档</b><small>已完成</small></span></div><div className="active"><i>2</i><span><b>材料准备</b><small>进行中</small></span></div><div><i>3</i><span><b>提交申报</b><small>待处理</small></span></div><div><i>4</i><span><b>结果归档</b><small>待处理</small></span></div></div>
    </article>
  </section>
}
...
function CompanyProfile({onMatch,profileComplete}:{onMatch:(complete:boolean)=>void;profileComplete:boolean}){
  const [step,setStep]=useState(1);
  const [marketProof,setMarketProof]=useState(profileComplete?"行业客户清单及专项说明":"");
  const stepMeta=[
    {name:"基本信息",status:"已完成",complete:true,title:"企业基本信息",text:"用于判断注册地区、主体资格、行业与企业规模"},
    {name:"研发与经营",status:"已完成",complete:true,title:"研发与经营情况",text:"已补充近三年经营数据及连续两年研发投入"},
    {name:"资质能力",status:"已完成",complete:true,title:"企业资质与专业化能力",text:"用于判断科技型企业资质、细分市场年限及信用要求"},
    {name:"知识产权",status:profileComplete?"已完成":"待补充1项",complete:profileComplete,title:"知识产权与市场证明",text:"Ⅰ类知识产权和细分市场证明将影响专精特新资格判断"},
  ];
  return <section className="page-stack"><div className="profile-header"><div><span className="section-kicker">COMPANY PROFILE</span><p>演示企业字段已按照当前真实专精特新申报通知整理</p></div><div className={profileComplete?"completion complete":"completion profile-88"}><div><b>{profileComplete?"100%":"88%"}</b><small>{profileComplete?"已完成 31/31 项":"已完成 28/31 项"}</small></div><span><i/></span></div></div><div className="profile-demo-summary"><div><span>虚拟演示企业</span><h3>北京星云科技有限公司</h3><p>北京市海淀区 · 软件和信息技术服务业 · 成立4年 · 68人</p></div><div className="profile-summary-tags"><span>科技型中小企业</span><span>主营业务占比86%</span><span>研发投入13%</span><span className="warning">Ⅰ类知识产权0项</span></div><small>企业为虚拟数据；匹配政策为真实政府政策</small></div><div className="profile-steps">{stepMeta.map((item,i)=><button onClick={()=>setStep(i+1)} className={step===i+1?"active":item.complete?"done":""} key={item.name}><span>{item.complete?"✓":i+1}</span><b>{item.name}</b><small>{item.status}</small></button>)}</div><div className="profile-form-card"><div className="form-card-title"><span>0{step}</span><div><h3>{stepMeta[step-1].title}</h3><p>{stepMeta[step-1].text}</p></div><i>企业数据仅用于政策匹配</i></div>{step===1&&<div className="form-grid"><label>企业名称<input defaultValue="北京星云科技有限公司"/></label><label>统一社会信用代码<input defaultValue="91110108MA01XXXXXX"/></label><label>注册地区<input defaultValue="北京市海淀区"/></label><label>成立日期<input defaultValue="2021-06-18"/></label><label>所属行业<input defaultValue="软件和信息技术服务业"/></label><label>企业规模<select defaultValue="中小企业"><option>中小企业</option></select></label><label>独立法人资格<select defaultValue="是"><option>是</option></select></label><label>企业职工总数（人）<input defaultValue="68"/></label></div>}{step===2&&<div className="form-grid"><label>2023年营业收入（万元）<input defaultValue="2180"/></label><label>2024年营业收入（万元）<input defaultValue="2750"/></label><label>2025年营业收入（万元）<input defaultValue="3280"/></label><label>2025年主营业务收入（万元）<input defaultValue="2820"/></label><label>主营业务收入占比<input defaultValue="86%"/></label><label>2025年末资产负债率<input defaultValue="42%"/></label><label>2024年研发费用<input defaultValue="325万元 / 占营收11.8%"/></label><label>2025年研发费用<input defaultValue="426万元 / 占营收13.0%"/></label></div>}{step===3&&<div className="form-grid"><label>科技型中小企业资质<select defaultValue="有效"><option>有效</option></select></label><label>资质有效期<input defaultValue="至2026-12-31"/></label><label>主导产品/细分市场<input defaultValue="企业知识管理与协同软件"/></label><label>细分市场经营年限<input defaultValue="4年"/></label><label>经营与信用状态<select defaultValue="正常"><option>正常</option></select></label><label>近三年重大安全、质量及环保事件<select defaultValue="无"><option>无</option></select></label><label>2023—2025年审计报告<select defaultValue="已上传并报备"><option>已上传并报备</option></select></label><label>综合评价结果<input defaultValue="由官方申报平台计算，运营人员仅做材料核查"/></label></div>}{step===4&&<><div className="form-grid"><label>软件著作权<input defaultValue="3项（Ⅱ类知识产权）"/></label><label className="profile-field-warning">已授权Ⅰ类知识产权<input defaultValue="0项"/></label><label>发明专利<input defaultValue="1项申请中，尚未授权"/></label><label>知识产权实际应用<input defaultValue="软著已用于主导产品"/></label><label>主导产品<input defaultValue="星云企业知识协同平台"/></label><label className={!marketProof.trim()?"profile-field-warning":""}>细分市场占有率证明<input value={marketProof} onChange={event=>setMarketProof(event.target.value)} placeholder="待上传行业排名、客户或第三方证明"/></label></div><div className="profile-gap-note"><span>!</span><div><b>{marketProof.trim()?"画像字段已补全，但仍有一项政策条件短板":"当前画像存在一项确定性短板和一项材料缺口"}</b><p>{marketProof.trim()?"细分市场证明已补充；企业仍无已授权Ⅰ类知识产权，匹配结果将继续标记为关键缺失。":"企业尚无已授权Ⅰ类知识产权；市场占有率证明待补充。即使基础条件较为匹配，也不能直接判断为完全符合专精特新认定条件。"}</p></div></div></>}<div className="form-actions"><button className="secondary">保存草稿</button><button className="form-submit" onClick={()=>step<4?setStep(step+1):onMatch(Boolean(marketProof.trim()))}>{step<4?"保存并继续":marketProof.trim()?"更新匹配结果":"生成初步匹配结果"} →</button></div></div></section>
}
...
function PolicyMatches({profileComplete,policyPublished,policyMatchReady}:{profileComplete:boolean;policyPublished:boolean;policyMatchReady:boolean}){
  ...
  return <section className="page-stack policy-matches-page">
    <div className="match-layout"><div className="match-list"><FilterRow labels={["全部结果 4","较高匹配 2","一般匹配 1","待补充信息 1"]}/>{visiblePolicies.map((item,i)=><button onClick={()=>setSelected(i)} className={`match-card ${selected===i?"selected":""}`} key={item.title}><div className="match-level-badge"><b>{item.match.replace("匹配","")}</b><small>规则匹配</small></div><div><span className="match-tags"><i>{item.region}</i><i>{item.demo?"演示数据":item.status}</i></span><h3>{item.title}</h3><p>{item.demo?item.reason:profileComplete?"注册地、经营年限、营收和研发投入等基础条件符合；Ⅰ类知识产权缺失，细分市场证明已补充待复核":item.reason}</p><small>{item.meta}</small></div><span className="match-arrow">›</span></button>)}</div>
      <aside className="diagnosis">{p.demo?<DemoPolicyDiagnosis policy={p}/>:<><div className="diagnosis-head"><span className="section-kicker">MATCH REPORT · 真实主线</span><h3>{p.title}</h3><div className="match-level"><b>{p.match}</b><span>规则初筛<small>建议人工复核</small></span></div></div><div className="condition-overview"><span><b>6</b><small>已满足</small></span><span><b>1</b><small>关键缺失</small></span><span><b>{profileComplete?0:1}</b><small>待补充</small></span></div><div className="condition-list"><Condition state="pass" title="注册与主体资格" text="北京市海淀区注册，独立法人"/><Condition state="pass" title="经营与研发条件" text="经营年限、营收、主营占比、研发投入和负债率符合基础门槛"/><Condition state="partial" title="Ⅰ类知识产权" text="当前0项，为需要人工核查的关键缺失条件"/>{profileComplete?<Condition state="pass" title="细分市场证明" text="已补充客户清单及专项说明，等待运营复核"/>:<Condition state="unknown" title="细分市场证明" text="尚未上传市场占有率、重点客户或第三方证明"/>}</div><button className="form-submit" disabled={profileComplete}>{profileComplete?"资料已补充，等待复核 ✓":"补充缺失信息 →"}</button><button className="consult-button" onClick={()=>setConsult("form")}>预约顾问人工复核</button><p className="diagnosis-note">一期不预测官方得分，也不自动判定申报资格。</p></>}</aside>
    </div>
    {consult!=="closed"&&!p.demo&&<ConsultModal state={consult} policy={p} onSubmit={()=>setConsult("success")} onClose={()=>setConsult("closed")}/>}
  </section>
}
...
function EnterpriseLeadDetail({name,profileComplete,onBack}:{name:string;profileComplete:boolean;onBack:()=>void}){
  ...
  return <section className="page-stack lead-detail"><div className="lead-detail-head"><button className="back-link" onClick={onBack}>← 返回企业列表</button><div><button className="secondary">分配负责人</button><button className="form-submit" onClick={()=>{setAdding(true);setSaved(false)}}>＋ 添加跟进记录</button></div></div>{saved&&<div className="success-banner"><span>✓</span><div><b>跟进记录已保存</b><p>企业状态已更新为“跟进中”，记录已写入服务时间线。</p></div><button onClick={()=>setSaved(false)}>×</button></div>}<div className="company-summary"><span className="company-logo">星</span><div><span className="lead-level">主演示企业</span><h2>{name}</h2><p>软件和信息技术服务业 · 北京市海淀区 · 成立4年 · 68人</p></div><dl><div><dt>画像完整度</dt><dd>{profileComplete?"100%":"88%"}</dd></div><div><dt>主演示政策</dt><dd>1项</dd></div><div><dt>规则初筛</dt><dd>较高匹配</dd></div><div><dt>负责人</dt><dd>周运营</dd></div></dl></div><div className="lead-detail-grid"><section className="lead-main-stack"><section className="panel"><div className="panel-head"><div><span className="section-kicker">COMPANY PROFILE</span><h3>企业画像摘要</h3></div><button>查看完整画像 →</button></div><div className="profile-tags"><span>科技型中小企业</span><span>软件和信息服务</span><span>细分市场4年</span><span>主营占比86%</span><span>研发投入13%</span>{profileComplete&&<span>细分市场证明已补充</span>}<span className="missing">Ⅰ类知识产权0项</span></div><div className="detail-grid"><DetailItem label="上年度营收" value="3,280万元"/><DetailItem label="资产负债率" value="42%"/><DetailItem label="研发费用" value="426万元"/><DetailItem label="知识产权" value="3项软著"/></div></section><section className="panel lead-match-diagnosis simplified"><div className="lead-match-head"><div><span className="section-kicker">PHASE 1 POLICY MATCH</span><h3>重点政策匹配结果</h3></div><span>真实政策</span></div><div className="diagnosis-policy-title simple"><div className="simple-match-badge"><b>较高</b><small>规则匹配</small></div><div><span>建议补充材料后人工复核</span><h4>{REAL_POLICY_TITLE}</h4><p>北京市经济和信息化局 · 资质认定 · 2026年8月31日24:00截止</p></div></div><div className="simple-match-groups"><article className="matched"><span>✓</span><div><b>主要条件已满足</b><p>北京注册、科技型中小企业、经营年限、营收、研发投入和资产负债率等基础条件符合。</p></div></article>{profileComplete?<article className="matched"><span>✓</span><div><b>已补充：细分市场证明</b><p>企业已提交客户清单及专项说明，等待运营人员复核材料有效性。</p></div></article>:<article className="pending"><span>?</span><div><b>待补充：细分市场证明</b><p>需上传重点客户、市场占有率或第三方证明，由运营人员确认。</p></div></article>}<article className="missing"><span>!</span><div><b>关键缺失：Ⅰ类知识产权</b><p>当前仅有3项软件著作权；需核查是否有已授权发明专利。</p></div></article></div><div className="phase-one-review-note"><span>1</span><p><b>系统自动初筛</b>：根据企业画像与政策条件生成“符合、缺失、待确认”结果。</p><i>→</i><span>2</span><p><b>运营人员复核</b>：确认关键条件后，再将结果展示给企业。</p></div><div className="followup-advice simple"><p>一期不预测官方评分，也不自动判定申报资格；结果仅用于政策筛选和运营跟进。</p><div><button className="secondary" onClick={()=>window.open(REAL_POLICY_URL,"_blank","noopener,noreferrer")}>查看政府网站原文 ↗</button><button className="form-submit" onClick={()=>{setAdding(true);setSaved(false)}}>添加跟进记录</button></div></div></section></section><aside className="panel service-timeline"><div className="panel-head"><h3>服务跟进时间线</h3><button>全部记录</button></div>{saved&&<Timeline time="刚刚" title="电话跟进已完成" text="已联系企业负责人，约定明天下午进一步沟通申报条件。"/>}<Timeline time="今天 15:42" title="企业提交人工咨询" text="希望确认专精特新政策的Ⅰ类知识产权条件"/><Timeline time="今天 14:18" title="完成企业测评" text={`画像完整度由64%提升至${profileComplete?"100%":"88%"}`}/><Timeline time="8月20日 10:35" title="AI政策问答" text="咨询2026年第二批专精特新资质申报"/>{adding&&<div className="followup-form"><h4>添加跟进记录</h4><label>跟进方式<select><option>电话沟通</option><option>微信沟通</option><option>线下面谈</option></select></label><label>跟进结果<textarea defaultValue={profileComplete?"核查Ⅰ类知识产权情况，并复核已提交的细分市场证明。":"核查Ⅰ类知识产权情况，并协助准备细分市场占有率证明。"}/></label><div><button className="form-back" onClick={()=>setAdding(false)}>取消</button><button className="form-submit" onClick={()=>{setAdding(false);setSaved(true)}}>保存记录</button></div></div>}</aside></div></section>
}
